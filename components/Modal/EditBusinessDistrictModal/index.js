...
export default function EditBusinessDistrictModal(props) {
    const { onHide, edit_business_district, businessDistrictSelected } = props
    ...
    useEffect(() => {
        if (businessDistrictSelected) {
            setBusinessDistrictForm({
                name: businessDistrictSelected.name,
    ...
    const editBusinessDistrict = () => {
        const newErrors = findFormDataErrors()

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors)
        } else {
            if (businessDistrictForm.image_url) {
                edit_business_district(businessDistrictForm)
                onHide()
            } else {
