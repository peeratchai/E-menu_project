...
export default function AdminEditProfileModal(props) {
        ...
        if (profile) {
            ...
            if (profile.roles.length > 0) {
                profile.roles.forEach((role) => {
                    if (role === 'employee' || role === 'parthner') {
    ...
    const saveProfile = async () => {
        ...
        const { first_name, last_name, gender, age, phoneNumber, profileImage, username, roles, is_active, restaurant_employee } = profileForm
        ...
        let responseProfile = await profileService.adminEditUserProfile(data)
