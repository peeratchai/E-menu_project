...
export const Menu = (categoryList, selected) =>
  categoryList.map((el, index) => {
    ...
    return (
      <MenuItem text={categoryName} key={categoryName} selected={selected} />
    );
  });
...
export default function RestaurantDetailWeb(props) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (
        selected &&
        categoryList[indexOfCategory].categoryName !== visibleSection
      ) {
  ...
  const activeCategory = () => {
    ...
    let tempCategoryNav = categoryList.map((category, index) => (
      <div
        ref={(elementRef) => (refEachCategory.current[index] = elementRef)}
        className={
          setClassNameCategoryNav(category, index) +
          " " +
          utilStyles.font_size_md +
          " " +
          styles.category_nav
        }
    ...
    let categorySection = categoryList.map((category, categoryIndex) => {
      ...
      return (
        <div
          id={category.name}
          ref={(categoryRef) =>
            (refsCategory.current[categoryIndex] = categoryRef)
          }
        ...
        >
          <Row className={styles.category_section}>
            <Col xs={12}>
              <div
                className={
                  utilStyles.font_size_xl + " " + styles.categoryHeader
                }
              >
                {category.name}
              </div>
              ...
              <Card.Text className={styles.card_text}>
                ...
                <Row style={{ marginTop: "15px" }}>
                  <Col md={8}>
                    ...
                    <div className={styles.menu_list}>{menuEachCategory}</div>
                  </Col>

                  <Col
                    md={4}
                    className={utilStyles.font_size_md}
