...
export default function Promote() {
    ...
    const addPromotion = async (event) => {
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            let data = {
                // restaurant ID
                restaurant: '7a5a8a5e-220d-40b9-91a7-b16c7196d77e',
                promote_content: promotedContents,
                title: bannerText,
                image: promotedImage
            }
            let accessToken = await checkLogin()
            // let response = await uploadService.uploadImage(promotedImage, accessToken)
            // console.log('response', response)
            try {
                let response = await partnerSerivce.addPromotion(data, accessToken)
    ...
    let promoteComponent
    if (isMobileResolution) {
        promoteComponent = (
            <MobileComponent
                promote_image_url={promoteImageUrl}
