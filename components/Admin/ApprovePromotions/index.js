...
export default function ApprovePromotions(props) {
    ...
    useEffect(() => {
        if (current_tab === 'approvePromotion') {
            getAllPromotions()
        }
    }, [current_tab])
    ...
    const getAllPromotions = async () => {
        setLoading(true)
        let promotions = await adminService.getAllPromotionsAdmin()
        ...
        if (pendingPromotions.length > 0) {
            pendingPromotions.forEach((promotions, index) => {
                pendingPromotionsDataTable.push({
                    ...
                    restaurant: promotions.restaurant,
    ...
    const confirmApprovePromotion = async (promotionsId) => {
        let data = {
            status: 'approved'
        }
        ...
        },
        {
            ...
            render: (promotion, record) => (
                <Space size="middle">
                    ...
                    <Popconfirm
                        title="Are you sure to approve promotion?"
                        onConfirm={() => confirmApprovePromotion(promotion.promotionId)}
                        okText="Yes"
                        cancelText="No"
                    >
                    ...
                    <Popconfirm
                        title="Are you sure to reject promotion?"
                        onConfirm={() => confirmRejectPromotion(promotion.promotionId)}
                        okText="Yes"
                        cancelText="No"
                    >
            ...
            <Spin spinning={loading} tip='loading...'>
                <Tabs defaultActiveKey="pending" >
                    ...
                    <TabPane tab="Approved" key="approved">
                        <Table columns={columnsApprovePromotion} dataSource={approvedPromotions} scroll={{ x: 'max-content' }} />
                    </TabPane>
                    <TabPane tab="Reject" key="reject">
                        <Table columns={columnsApprovePromotion} dataSource={rejectPromotions} scroll={{ x: 'max-content' }} />
                    </TabPane>
                </Tabs>
            </Spin>
            <EditPromoteModal
                ...
                restaurant_id={promotionSelected.restaurant.id}
                get_promotions={getAllPromotions}
            />
